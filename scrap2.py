# -*- coding: utf-8 -*-
"""
Created on Sat Jan  2 20:14:42 2021
...
len_results = 0
print(len_results)
all_results = driver.find_elements_by_xpath(JOB_RESULTS_BAR)
print(len(all_results))
while len(all_results)>len_results:
    for r in all_results[len_results:]:
        r.click()
        sleep(1)
        description = driver.find_element_by_xpath(DESCRIPTION).text
        location = driver.find_element_by_xpath(LOCATION).text
        company = driver.find_element_by_xpath(COMPANY).text
        title = driver.find_element_by_xpath(JOB_TITLE).text
        criteria = driver.find_element_by_xpath(JOB_CRITERIA).text
    ...
    all_results = driver.find_elements_by_xpath('//*[@id="main-content"]/div/section/ul/li')
    print(len_results)
    print(len(all_results))
